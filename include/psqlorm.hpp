...
class PSQLORM {
 public:
  PSQLORM() : m_connection(std::move(std::unique_ptr<DatabaseConnection>{new DatabaseConnection}))
  {
  ...
  PSQLORM(std::unique_ptr<DatabaseConnection> db_connection, DatabaseConfiguration config)
